/**
 * Proxy seguro para a API do Monday.com
 * -----------------------------------------------------------
 * Por que esse servidor existe:
 * O Confere (frontend) roda inteiro no navegador. Qualquer token
 * colocado no código do frontend fica visível pra quem inspecionar
 * a página. Esse servidor guarda o token do Monday como variável
 * de ambiente (nunca no código) e expõe só os dados que o Confere
 * precisa, através de rotas próprias.
 *
 * O Confere nunca fala com api.monday.com diretamente — ele fala
 * com ESTE servidor, e este servidor fala com o Monday.
 *
 * -----------------------------------------------------------
 ...
 *   4) npm start
 *
 * Como fica em produção: hospede isso em qualquer lugar que
 * suporte Node (Render, Railway, Fly.io, uma VM, etc.) e configure
 * MONDAY_API_TOKEN nas variáveis de ambiente do serviço — nunca
 * como arquivo commitado no git.
 */
...
const app = express();
app.use(cors()); // em produção, restrinja para o domínio do Confere
...
// recarregar a página ficar instantâneo.
