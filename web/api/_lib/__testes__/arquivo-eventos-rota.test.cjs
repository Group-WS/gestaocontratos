/* O registro dos arquivos da obra pela API (23/09/2026) — mesmo molde do de importacoes.
 *
 * Roda com: node web/api/_lib/__testes__/arquivo-eventos-rota.test.cjs
 *
...
const { rotasDeArquivoEventos } = require(path.join(__dirname, "..", "rotas", "arquivoEventos.js"));
...
global.fetch = async (url, opcoes = {}) => {
  ...
  if (u.pathname === "/rest/v1/obra_arquivo_evento") {
    ...
    return responder([{ id: 1, acao: "anexou", tipo: "avulso", arquivo_nome: "a.pdf" }]);
  }
  return json([], 404);
};
...
const REGISTRO = {
  acao: "trocou", tipo: "marcenaria", titulo: "Caderno de Marcenaria",
  arquivoNome: "marcenaria-v2.pdf", arquivoAnterior: "marcenaria.pdf", caminho: "2519/marcenaria/x.pdf",
};
...
const servidor = app.listen(0, async () => {
  ...
  try {
    ...
    let r = await pedir("t-gc", "POST", "/api/obras/2519/arquivos-eventos", REGISTRO);
    conf("o GC da obra registra", r.status, 200);
    conf("... com os nomes das colunas e o autor do LOGIN", recebido, {
      obra_codigo: "2519", acao: "trocou", tipo: "marcenaria", titulo: "Caderno de Marcenaria",
      arquivo_nome: "marcenaria-v2.pdf", arquivo_anterior: "marcenaria.pdf", caminho: "2519/marcenaria/x.pdf",
      autor: "gc@groupws.com.br",
    });

    r = await pedir("t-gc", "POST", "/api/obras/2519/arquivos-eventos", { ...REGISTRO, autor: "outro@groupws.com.br" });
    conf("autor no corpo do pedido nem chega ao banco", r.status, 400);
    r = await pedir("t-gc", "POST", "/api/obras/2519/arquivos-eventos", { ...REGISTRO, tipo: "aditivo" });
    conf("tipo inventado para antes do banco", r.status, 400);
    r = await pedir("t-gc", "POST", "/api/obras/2600/arquivos-eventos", REGISTRO);
    conf("obra de outro GC: nao encontrada", r.status, 404);
    r = await pedir("t-mehoo", "POST", "/api/obras/2519/arquivos-eventos", REGISTRO);
    conf("quem so' acompanha nao registra", r.status, 404);

    console.log("\n=== ler ===");
    r = await pedir("t-mehoo", "GET", "/api/obras/2519/arquivos-eventos");
    conf("quem enxerga a obra le' o registro", [r.status, r.corpo.eventos.length], [200, 1]);
    conf("... do mais novo pro mais antigo, com limite", [recebido.order, recebido.limit], ["criado_em.desc", "200"]);
    conf("... pedindo as colunas uma a uma", recebido.select.includes("*"), false);

    console.log("\n=== o SQL ainda nao rodou ===");
    temTabela = false;
    r = await pedir("t-gc", "GET", "/api/obras/2519/arquivos-eventos");
    conf("a leitura devolve lista vazia, sem erro", [r.status, r.corpo], [200, { semTabela: true, eventos: [] }]);
    r = await pedir("t-gc", "POST", "/api/obras/2519/arquivos-eventos", REGISTRO);
    conf("o registro diz qual SQL falta", [r.status, /obra-arquivo-evento\.sql/.test(r.corpo.error)], [503, true]);
    temTabela = true;
  } finally {
    servidor.close();
